...
class CalendarYvv{
	...
	createCalendar(){
		...
		var cntCale = $("<div class='calendar-yvv w-100' style='font-size: xx-large;'>");
	...
	createHeaderM(){
		...
		var arrowL = $("<span>").html("");
		var arrowR = $("<span>").html("");
		...
		var vYear = new Date(this.diaSeleccionado).getFullYear() + 543;
		var vMonth = new Date(this.diaSeleccionado).getMonth();
		var months_th = [ "มกราคม", "กุมภาพันธ์", "มีนาคม", "เมษายน", "พฤษภาคม", "มิถุนายน", "กรกฎาคม", "สิงหาคม", "กันยายน", "ตุลาคม", "พฤศจิกายน", "ธันวาคม", ];
		var title = $("<span class='text-uppercase'>").html(months_th[vMonth] + ' ' + vYear);
		var _this = this;
	...
	createDayTag(){
		...
		var arrEngtoTh = {"Mon": "จ.", "Tue": "อ.", "Wed": "พ.", "Thu": "พฤ.", "Fri": "ศ.", "Sat": "ส.", "Sun": "อา."};
		diasOrd.fechCort.forEach(function(e){			
			var div = $("<div class='d-flex border flex-fill w-100 justify-content-center p-2'>").html(arrEngtoTh[e]);
			cont.append(div);
		});
		return cont;
	}
	createDaysMont(){
		...
		for(var i=0;i<this.cantDias;i++){
			...
			var div = $("<div class='d-flex border flex-fill w-100 justify-content-center pt-3 pb-3 btn "+this.btnD+"' data-date='"+fechNow+"' style='font-size: xx-large;'>").html(i+1);
			var clas_e = this;
			var _ind = (this.cantDias+posMes)%7;

			//dia seleccionado
			if(this.diaSeleccionado==fechNow){
				div = $("<div class='current-date-selected d-flex border flex-fill w-100 justify-content-center pt-3 pb-3 btn "+this.btnD+"' data-date='"+fechNow+"' style='font-size: xx-large;'>").html(i+1);
			}
			//dias resaltados o importantes
			if(this.diasResal.indexOf(i+1)!=-1){
				div = $("<div class='d-flex border flex-fill w-100 justify-content-center pt-3 pb-3 btn "+this.btnD+"' data-date='"+fechNow+"' style='background: "+this.colorResal+"; color: "+this.textResalt+"; font-weight: bold;font-size: xx-large;'>").html(i+1);
			}
